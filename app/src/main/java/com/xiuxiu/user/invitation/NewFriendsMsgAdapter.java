...
import com.xiuxiu.easeim.DataSyncManager;
...
public class NewFriendsMsgAdapter extends ArrayAdapter<InviteMessage> {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (msg != null) {
			...
			XiuxiuUserInfoResult xiuxiuUserInfoResult = EaseUserCacheManager.getInstance().getBeanById(msg.getFrom());
			if(xiuxiuUserInfoResult!=null && !TextUtils.isEmpty(xiuxiuUserInfoResult.getXiuxiu_name())){
				holder.name.setText(xiuxiuUserInfoResult.getXiuxiu_name());
			}
	...
	private void acceptInvitation(final Button buttonAgree, final Button buttonRefuse, final InviteMessage msg) {
		...
		new Thread(new Runnable() {
			public void run() {
				// 咻咻自身服务器加好友
				boolean isSuccess = DataSyncManager.getInstance().beFriends(msg.getFrom());
				if(!isSuccess){
